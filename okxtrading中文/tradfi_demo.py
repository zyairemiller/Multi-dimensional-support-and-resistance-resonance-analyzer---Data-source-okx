...
SELECTOR_HTML = _SELECTOR_PATH.read_text(encoding="utf-8") if _SELECTOR_PATH.exists() else "<html><body><h1>找不到 _tradfi_selector.html</h1></body></html>"
...
def _run_analysis(inst_ids):
    """后台分析线程：为每个 inst_id 构造临时配置，调用分析"""
    import okxtrading
    ...
    try:
        ...
        for i, inst_id in enumerate(inst_ids):
            ...
            DemoServable._progress["msg"] = f"分析中: {inst_id} ({i+1}/{len(inst_ids)})"
            try:
                r = analyze_instrument(base, refresh=True)
                if r:
                    results.append(r)
            except Exception as e:
                print(f"  {inst_id} 分析失败: {e}")

        DemoServable._progress["pct"] = 90
        DemoServable._progress["msg"] = "生成报告中..."
        if results:
            ...
            DemoServable._progress["msg"] = f"完成! 共 {len(results)} 个品种"
            DemoServable._progress["done"] = True
            DemoServable._progress["report_url"] = f"/{out.name}"
        else:
            DemoServable._progress["msg"] = "所有品种分析失败"
            DemoServable._progress["done"] = True
    finally:
        # 恢复原配置和 DB
